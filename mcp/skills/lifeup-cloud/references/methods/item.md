...
| set_icon         | Set icon            | emoji, http(s) URL, or built-in sample name | ☕ | No | Same as `icon` on create. Unsupported values return `unsupported_parameter`. |
...
| effects          | Use effects         | JSON text            | `help` `item_structures` § Item Effects | No | Set item usage effects |
| purchase_limit   | Restriction rules   | JSON text            | `help` `item_structures` § Purchase Limit | No | Pass `null` to clear all restrictions |
...
?> Adjust loot box contents on an existing box item: `help` `loot_box` or `help` `loot_box/v2`. Define a new box via `effects` type 7: `help` `item_structures` § Item Effects.
